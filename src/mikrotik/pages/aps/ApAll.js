...
export default function ApAll() {
  ...
  const { data, isLoading } = useGet_AP_nodo_filterQuery({ access: user.access, search: search, routerName: routerNombre, ordering: ordenar })
          ...
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="py-2 px-4 border-b text-xs text-center">
                ...
                <button onClick={handleOrdenar} className={`${ordenar == 'frecuencia' && 'text-decoration-line: overline'}${ordenar == '-frecuencia' && 'text-decoration-line: underline'}`}> Frecuencia *</button>
              </th>
